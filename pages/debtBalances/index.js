...
const MyComponent = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const balances = event.target.elements.balances.value
      .split(",")
      .map(Number);
    const monthlyPayments = event.target.elements.monthlyPayments.value
      .split(",")
      .map(Number);

    // Create an array of debts from the balances and monthly payments
    const newDebts = balances.map((balance, index) => ({
      balance,
